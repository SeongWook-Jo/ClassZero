...
public class csPhotonInit : MonoBehaviour
{
    ...
    void Awake()
    {
        if (!PhotonNetwork.connected)
        {
            ...
            PhotonNetwork.playerName = "GUEST" + Random.Range(1, 9999); // 이름 랜덤 생성
        }

        roomName.text = "ROOM_" + Random.Range(0, 999).ToString("000"); //룸이름 무작위

        scrollContents.GetComponent<RectTransform>().pivot = new Vector2(0.0f, 1.0f); //스크롤 좌표 생성
    }
    ...
    void OnJoinLobby()
    {
        ...
        //PhotonNetwork.JoinRandomRoom(); // (UI 버전에서는 주석 처리)
        //userId.text = GetUserId();  //유저 아이디 가져와 셋팅
    }

    string GetUserId() //로컬에 저장된 플레이어 이름을 반환하거나 랜덤 생성하는 함수 (UI 버전에서 사용)
    {
        string userId = PlayerPrefs.GetString("USER_ID");
    ...
    void OnPhotonCreateRooomFailed(object[] codeAndMsg)
    {
        Debug.Log(codeAndMsg[0].ToString());
